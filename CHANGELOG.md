...
- Hash-lock every workflow dependency install: three compiled requirement sets under `requirements/ci/` (`gate-tools` for the YAML the slice gate and bootstrap read, `dev-env` mirroring the pyproject dev extra, `build-tools` mirroring `[build-system].requires`), installed everywhere with `pip install --require-hashes`; the package itself installs with `--no-build-isolation --no-deps -e .` so no job resolves a third-party package outside the pinned sets. Adopted from the law proven in Vaquum/Limen (its PR #729).
