...
- Harden the closeout guard the same way: only successful required check runs become evidence, an ambiguous duplicate Done Means section fails the writer instead of receiving evidence, and a no-PR close whose hand-typed Merge SHA is not reachable from `main` is reopened.
